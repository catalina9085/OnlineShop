...
//
//        for (User user : users) {
//            if (user.getNotifications() != null && user.getNotifications().isViaPush()) {
...
//        }
//
//        System.out.println("Test notifications sent to all users.");
//    }
	...
	public void notifyAboutOffer(Product product) {
		...
		for(User user:users) {
			if(user.getNotifications()!=null && user.getNotifications().isOffers()) {
				...
				if(user.getNotifications().isViaPush()) {
					String accessProduct="/product/"+product.getId();
