...
const realBtn=document.getElementById("files");
const customBtn=document.getElementById("customUploadButton");
const customText=document.getElementById("customUploadText");
...
realBtn.addEventListener("change",function(){
	if(realBtn.files.length==1){
		 customText.innerHTML=realBtn.files[0].name;
	}
	else if (realBtn.files.length > 0) {
	     customText.innerHTML = `${realBtn.files.length} files`;
	} else
